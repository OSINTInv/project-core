...
import {
  ...
  ExternalLink, Globe, Save
} from "lucide-react"
import { formatSizeMb } from "@/lib/utils"
import { getAcquisitionMeta } from "@/lib/acquisition"
import { useCORE } from "@/context/CoreContext"
import { useProfiles } from "@/context/ProfileContext"
import { SaveCOREDialog } from "@/components/SaveCOREDialog"
import type { SavedPack } from "@/context/ProfileContext"
...
export default function Builder() {
  ...
  const { items: coreItems, removeItem, totalSizeMb, categories: coreCategories, addItem, hasItem, clear } = useCORE()
  const { createProfile } = useProfiles()

  // Server-side profile creation (community sharing) — still available but not primary
  const _createServerProfile = useCreateProfile()
  ...
  const selectedPackObjects: SavedPack[] = Array.from(selectedPacks).map(pid => {
    const pack = packsData?.find(p => p.id === pid)
    return {
      id: pid,
      name: pack?.name ?? `Pack #${pid}`,
      slug: pack?.slug ?? `pack-${pid}`,
      approximateTotalSizeMb: pack?.approximateTotalSizeMb ?? null,
    }
  })

  const totalPackSize = selectedPackObjects.reduce((sum, p) => sum + (p.approximateTotalSizeMb ?? 0), 0)
  const estimatedTotal = totalSizeMb + totalPackSize

  const handleSaveCORE = (input: Parameters<typeof createProfile>[0]) => {
  ...
  return (
    <>
        ...
        <div className="bg-card border border-border rounded-sm p-6 md:p-8">
          ...
          {step === 1 && (
            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
              <h2 className="text-xl font-bold font-mono border-b border-border pb-2 mb-6">01. DEFINE YOUR CORE</h2>
              ...
              <div className="pt-6 mt-6 border-t border-border flex justify-end">
                <Button onClick={() => setStep(2)} className="font-mono font-bold">
                  CURATE RESOURCES <ArrowRight className="w-4 h-4 ml-2" />
                </Button>
              </div>
            </div>
          )}
          ...
          {step === 2 && (
            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
              <div className="flex justify-between items-end border-b border-border pb-2 mb-6">
                <h2 className="text-xl font-bold font-mono">02. CURATE RESOURCES</h2>
                <span className="font-mono text-xs text-primary">{coreItems.length} Selected</span>
              </div>
              <p className="text-sm text-muted-foreground mb-4">Select the information, knowledge, software, and tools you want offline. Resources added from the Atlas are already selected.</p>
                  ...
                  return (
                    <div key={res.id}
                      onClick={() => inCore ? removeItem(res.id) : addItem({
                        id: res.id, name: res.name, slug: res.slug,
                        category: res.category, categoryName: res.categoryName,
                        approximateSizeMb: res.approximateSizeMb ?? null,
                        offlineCapability: res.offlineCapability,
                        resourceType: res.resourceType, description: res.description,
                      ...
                      className={`p-3 border rounded-sm flex items-center gap-4 cursor-pointer transition-colors ${inCore ? 'border-primary bg-primary/10' : 'border-border bg-background hover:border-primary/50'}`}
                    >
                      <div className={`w-5 h-5 rounded-full border flex items-center justify-center shrink-0 ${inCore ? 'border-primary bg-primary text-primary-foreground' : 'border-muted-foreground/50'}`}>
              ...
              <div className="pt-6 mt-6 border-t border-border flex justify-between">
                ...
                <Button onClick={() => setStep(3)} className="font-mono font-bold">
                  ADD PACKS <ArrowRight className="w-4 h-4 ml-2" />
                </Button>
              </div>
            </div>
          )}
              ...
              <div className="h-[400px] overflow-y-auto pr-2 space-y-3">
                {packsLoading ? (
                  ...
                  <div key={pack.id} onClick={() => togglePack(pack.id)}
                    className={`p-4 border rounded-sm flex gap-4 cursor-pointer transition-colors ${selectedPacks.has(pack.id) ? 'border-primary bg-primary/10' : 'border-border bg-background hover:border-primary/50'}`}
                  >
                    <div className="pt-1 shrink-0">
                      <div className={`w-5 h-5 rounded-full border flex items-center justify-center ${selectedPacks.has(pack.id) ? 'border-primary bg-primary text-primary-foreground' : 'border-muted-foreground/50'}`}>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-[10px] font-mono text-muted-foreground">{formatSizeMb(pack.approximateTotalSizeMb || 0)}</div>
                    </div>
                  </div>
                ))}
              </div>
          ...
          {step === 4 && (
            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
              <h2 className="text-xl font-bold font-mono border-b border-border pb-2 mb-6">04. REVIEW & SAVE</h2>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-6">
                  {coreCategories.length > 0 && (
                    <div>
                      <h3 className="text-xs font-mono font-bold uppercase text-muted-foreground mb-3">Categories in this CORE</h3>
                  ...
                  {formData.name && (
                    <div>
                      ...
                      {formData.authorName && <p className="text-sm text-muted-foreground">by {formData.authorName}</p>}
                    </div>
                  )}
                  <div>
                    ...
                    <dl className="text-sm font-mono space-y-1">
                      {formData.purpose && <div className="flex justify-between"><dt className="text-muted-foreground">Purpose:</dt><dd className="capitalize">{formData.purpose}</dd></div>}
                      {formData.targetDevice && <div className="flex justify-between"><dt className="text-muted-foreground">Device:</dt><dd>{formData.targetDevice}</dd></div>}
                    </dl>
                  </div>
                </div>
                ...
                <div className="bg-background border border-border p-6 rounded-sm space-y-4">
                  ...
                  {formData.storageCapacityGb && (estimatedTotal > Number(formData.storageCapacityGb) * 1024) && (
                    <div className="p-3 bg-[#F97316]/10 border border-[#F97316]/30 text-[#F97316] text-xs font-mono rounded-sm mt-4">
                      Estimated size exceeds your available storage ({formData.storageCapacityGb} GB).
                    </div>
                  )}
                </div>
              </div>
              ...
              {coreItems.length === 0 && (
                <div className="p-4 bg-muted/30 border border-border rounded-sm text-center">
                  <p className="text-sm text-muted-foreground font-mono">
                    No resources selected. <Link href="/atlas" className="text-primary hover:underline">Browse the Atlas</Link> to add resources.
                  </p>
                </div>
              )}

              <div className="pt-6 mt-6 border-t border-border flex justify-between">
                <Button variant="outline" onClick={() => setStep(3)} className="font-mono font-bold">
                  <ArrowLeft className="w-4 h-4 mr-2" /> BACK
                </Button>
                <Button
                  onClick={() => setShowSaveDialog(true)}
                  disabled={coreItems.length === 0}
                  className="font-mono font-bold tracking-wider gap-2"
                >
      ...
      <SaveCOREDialog
        ...
        resources={coreItems}
        packs={selectedPackObjects}
        estimatedStorageMb={estimatedTotal}
        prefill={{
          name: formData.name,
